...
## Honest scope notes — what's a real implementation vs. the closest practical equivalent

A few of the originally requested items aren't reliably doable in a generic, cross-platform desktop app without extra native tooling that can't be verified without a real OS to test on. Here's what shipped instead, and why:

- **Lip-sync** now has two paths. With "Real voice (API TTS)" enabled, speech is generated by OpenAI TTS or ElevenLabs, the audio is decoded, and the mouth is driven by a live `AnalyserNode` reading actual loudness — real amplitude-based sync. With it off (the default), it falls back to the browser's Speech Synthesis, where there's no raw audio buffer to analyze, so the mouth is driven from word-boundary timing instead — convincing but not frame-accurate. The real-TTS path is the better one; it just needs an OpenAI or ElevenLabs key.
- **Voice input** uses Chromium's built-in Speech Recognition (the same engine behind Chrome's dictation), not a bundled offline Whisper model. A real Whisper integration means shipping/loading a few hundred MB model file and adding a transcription pipeline — out of scope for "add this in one go," but a reasonable follow-up if you want fully offline voice.
- **"React to a long-running terminal command finishing"** isn't something a generic app can detect cross-platform without hooking into a specific terminal emulator or shell — there's no OS-agnostic signal for it. Not implemented.
- **Battery warnings** were scoped down to **CPU load warnings**, using Node's built-in `os.loadavg()` — a real battery-level API needs a native module (e.g. `systeminformation`) that adds install complexity and platform-specific binaries this project doesn't currently depend on.
- **"Watch the active window"** (e.g. knowing which app you have focused) needs a native module too (commonly `active-win`), which has its own per-platform binaries. It's not wired in by default; clipboard watching is implemented instead as the working "system awareness" channel. If you want active-window context later, `npm install active-win` and a small main.js addition would slot into the existing `system:*` IPC pattern.
- **Footstep/landing animations** weren't added since there's no locomotion system yet (the character doesn't currently walk across the screen) — there's nothing for footstep animation to attach to until a "walk to a point" behavior exists.
- **Outfit/accessory layering** is implemented generically as a part visibility toggle list (works on any VRM with separate mesh objects), not a deeper "outfit preset" system, since real outfit swapping depends on how each specific model's meshes/materials are structured.
